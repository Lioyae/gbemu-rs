...
Windows Terminal、现代 PowerShell 终端或同等能力的终端较适合运行。进入游戏模式前，
程序会自动尝试将终端扩大到最低尺寸；如果当前屏幕或字体限制导致扩容失败，会显示明确的
中文尺寸错误。此时可以缩小终端字体或手动扩大窗口后重试。
...
`roms/`，会自动将其加入 ROM 库。启动器配置保存在操作系统的用户配置目录中。双击启动
时如果发生 ROM 加载、终端初始化或尺寸错误，错误窗口会等待按下 Enter，不会立即关闭。
...
十个即时存档槽位的状态。损坏 ROM 会显示为红色错误项，详情区域提供解析原因，且
不会阻止其他文件显示。
